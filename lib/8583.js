...
class Main {
        ...
        if (bitmap[i] === 1) {
          ...
          if (this_format) {
            if (this_format.LenType === 'fixed') {
              ...
              if (
                this.Msg[subField] &&
                this.Msg[subField].length > this_format.MaxLen
              )
                return {
  ...
  hasSecondaryBitmap(primaryBitmapBuffer, config) {
    const bitmap = T.getHex(
      primaryBitmapBuffer.toString(config.bitmapEncoding || 'hex')
    )
      .split('')
      .map(Number);
    return bitmap[0] === 1;
  ...
  unpack_127(incoming, isoJSON, config) {
    if (Buffer.isBuffer(incoming)) {
      ...
      const bitmap = T.getHex(
        incoming.slice(4, bitmapEnd).toString(config.bitmapEncoding || 'hex')
      )
        .split('')
        .map(Number);
  ...
  getBufferMessage() {
    ...
    if (_0_127_Buffer.error) {
      ...
      let len_0_127_1 = this.getTCPHeaderBuffer(
        parseInt(Number(_0_127_Buffer.byteLength) / 256, 10)
      );
      let len_0_127_2 = this.getTCPHeaderBuffer(
        parseInt(Number(_0_127_Buffer.byteLength) % 256, 10)
      );
