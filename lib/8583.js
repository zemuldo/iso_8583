...
class Main {
  ...
  rebuildExtensions_127_25() {
    let data = this.Msg['127.25']

    // Hnalde quoted key value string eg 'key1='value1',key2="value2"'
    if (data.startsWith("'") || data.startsWith('"')) {
      this.embededProperties.field_127_25_quoted = true
    ...
    if (data) {
      this.upackFieldWithBitmap('127.25', 16)
    }
  ...
  upackFieldWithBitmap(parentField, bitmaLength) {
    ...
    let bitmap_127 = T.getHex(dataString.slice(0, bitmaLength))
      .split('')
      .map(Number);
  ...
  unpackKeyValueStringField(field) {
    ...
    return dataString.split('; ').reduce((_ignored, s) => {
      const kv = s.split('=')
  ...
  rebuildExtensions() {
    if (this.Msg['127']) {
      ...
      if (this.field127Encoding === 'key_value_encoded') {
        this.unpackKeyValueStringField('127')
      }
      else {
        // bitmap_encoded is default
        this.upackFieldWithBitmap('127', 16)
      }
  ...
  contractField(field) {
    ...
    if (field.length > 12 && field.length < 14) {
      return field + '.' + Number(field.split('field_127_')[1]);
