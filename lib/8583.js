...
class Main {
  ...
  getTType() {
    if (this.Msg['3'])
      return T.getTransType(this.Msg['3'].slice(0, 2));
    else
      return {
        error: 'transaction type not defined in message'
      };
  }
  ...
  getAccType() {
    if (this.Msg['3'])
      return T.getAccType(this.Msg['3'].slice(2, 4));
    else
      return {
        error: 'transaction type not defined in message'
      };
  ...
  getAccountTypeTo() {
    if (this.Msg['3'])
      return T.getAccType(this.Msg['3'].slice(4, 6));
    else
      return {
        error: 'transaction type not defined in message'
      };
  ...
  getTransStatus() {
    if (this.Msg['39'])
      return T.getTranStatus(this.Msg['39']);
    else
      return {
        error: 'transaction status not defined in message'
      };
  }
        ...
        if (this.bitmaps[i] === 1) {
          ...
          if (this_format) {
            if (this_format.LenType === 'fixed') {
              ...
              if (!this_format.MaxLen) return {
                error: 'max length not implemented for ' + this_format.LenType + field
              };
              if (this.Msg[field] && this.Msg[field].length > this_format.MaxLen) return {
                error: 'invalid length of data on field ' + field
              };
              if (thisLen === 0) {
                return {
  ...
  getMti() {
    ...
    if (state) {
      ...
      if (mti === null || mti === undefined) {
        return {
          error: 'mti undefined in message'
        };
      } else {
        if (this.checkMTI()) {
          let _mti;
          ...
          if (!this.Msg['0']) {
            if (!this.MsgType) {
              return {
                code: 667,
                error: 'mti undefined on field 0'
              };
            } else {
              _mti = this.MsgType;
            }
      ...
      for (let i = 0; i < bitmap_127.length; i++) {
        if (bitmap_127[i] === 1) {
          ...
          if (this_format.LenType === 'fixed') {
            ...
            if (thisLen === 0) {
              throw {
                error: 'field ' + field + ' format not implemented'
              };
            } else {
  ...
  assembleBitMap() {
    if (this.checkMTI()) {
      ...
      return {
        error: 'bitmap error, iso message type undefined or invalid'
      };
  }
  ...
  getBmpsBinary() {
    ...
    if (state.error) {
      ...
      if (!this.Msg['0']) {
        return {
          error: 'message type error, empty or undefined'
        };
      } else {
  ...
  getBitMapHex() {
    ...
    if (state.error) {
      ...
      if (this.bitmaps !== null && msgTypes(this.MsgType)) {
        ...
        return {
          error: 'bitmap error, expecting 128 length unit array'
        };
      }
    }
  }
        ...
        if (bitmap[i] === 1) {
          ...
          if (this_format) {
            if (this_format.LenType === 'fixed') {
              ...
              let thisLen = T.getLenType(this_format.LenType);
              if (!this_format.MaxLen) return {
                error: 'max length not implemented for ' + this_format.LenType + subField
              };

              if (this.Msg[subField] && this.Msg[subField].length > this_format.MaxLen) return {
                error: 'invalid length of data on field ' + subField
              };
              if (thisLen === 0) {
                throw {
                  error: 'field ' + subField + ' format not implemented'
                };
              } else {
                //check length of iso field
  ...
  unpack_127_1_63(slice_127, isoJSON) {
    ...
    let bitmap = T.getHex(slice_127.slice(0, 8).toString('hex')).split('').map(Number);
  ...
  hasSecondaryBitmap(primaryBitmapBuffer, config) {
    const bitmap = T.getHex(primaryBitmapBuffer.toString(config.bitmapEncoding || 'hex')).split('').map(Number);
    return bitmap[0] === 1;
  }
  ...
  unpack_127(incoming, isoJSON, config) {
    if (Buffer.isBuffer(incoming)) {
      ...
      if (!this._checkMTI(mti)) {
        return {
          error: 'failed to unpack at get mti'
        };
      }
      ...
      const bitmap = T.getHex(incoming.slice(4, bitmapEnd).toString(config.bitmapEncoding || 'hex')).split('').map(Number);
      ...
      return {
        error: 'expecting buffer but got ' + typeof incoming
      };
    }
  ...
  getIsoJSON(buffer, _config) {
    ...
    if (Buffer.isBuffer(buffer)) {
      ...
      return {
        error: 'expecting buffer but got ' + typeof buffer
      };
    }
  ...
  assemble127_extensions() {
    ...
    if (mtiCheck && validate && state) {
      ...
      return {
        error: 'Invalid Message in 127 extensions'
      };
  }
  ...
  addField(number, data) {
    ...
    if (!this_format)
      return {
        error: 'field ' + number + ' not implemented'
      };
  ...
  getJsonFromXml(string) {
    if (string) {
      ...
      return {
        error: 'xml is not properly encoded'
      };
    }
  ...
  getXMLString() {
    ...
    if (!this.MsgType || !msgTypes(this.MsgType)) {
      return {
        error: 'mti undefined or invalid'
      };
    } else {
