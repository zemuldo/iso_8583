...
class Main {
    ...
    getBufferMessage (obj){
        ...
        let tcpH1Buffer = Buffer.alloc(1,this.getTCPHeaderBuffer(obj.lengthIndicator[0]),'hex')
        let tcpH2Buffer = Buffer.alloc(1,this.getTCPHeaderBuffer(obj.lengthIndicator[1]),'hex')
        let bitmapBuffer = Buffer.alloc(16,obj.bitmapHex,'hex')
        let dataBuffer = Buffer(obj.dataString)

        return Buffer.concat([tcpH1Buffer,tcpH2Buffer,mtiBuffer,bitmapBuffer,dataBuffer])
