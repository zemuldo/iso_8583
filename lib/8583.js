...
class Main {
    ...
    unpack_127_25_1_63(slice_127_25,isoJSON){
        console.log("Bitmap for 127.25.0-63  >>>>> "+slice_127_25.slice(2, 10).toString('hex'))
        isoJSON['127.1'] = slice_127_25.slice(2, 10).toString('hex')
        let bitmap = getHex(slice_127_25.slice(2, 10).toString('hex')).split('').map(Number)
        slice_127_25 = slice_127_25.slice(10, slice_127_25.length)
    ...
    unpack_127_1_63(slice_127,isoJSON){
        console.log("Bitmap for 127.0-63  >>>>> "+slice_127.slice(2, 10).toString('hex'))
        let bitmap = getHex(slice_127.slice(2, 10).toString('hex')).split('').map(Number)
        console.log(bitmap.join(''))
        console.log(slice_127.byteLength)
        slice_127 = slice_127.slice(10, slice_127.length)
        console.log(slice_127.byteLength)
    ...
    unpack_127(incoming,isoJSON) {
        if(Buffer.isBuffer(incoming)){
            ...
            isoJSON['0'] = mti
            if(!this.checkMTI(isoJSON)){
                return {error:"failed to unpack at get mti"}
            }
            let bitmap = getHex(incoming.slice(4, 20).toString('hex')).split('').map(Number)
            console.log("Bitmap for 0-127  >>>>> "+incoming.slice(4, 20).toString('hex'))
            let thisBuff = incoming.slice(20, incoming.length)
            for (let i = 1; i < bitmap.length; i++) {
                    ...
                    if (formats[field]) {
                        if (formats[field].LenType=="fixed") {
                            if(formats[field].ContentType==="b"){
                                ...
                                thisBuff = thisBuff.slice(formats[field].MaxLen/2, thisBuff.length)
                            }
                            else {
                                isoJSON[field] = thisBuff.slice(0, formats[field].MaxLen).toString()
                                thisBuff = thisBuff.slice(formats[field].MaxLen, thisBuff.length)
                            }
                        }
                        else {
                            let thisLen = getLenType(formats[field].LenType)
                            ...
                            //check length of iso field
                                ...
                                thisBuff = thisBuff.slice(thisLen, thisBuff.length)
                                isoJSON[field] = thisBuff.slice(0, Number(len)).toString()
                                thisBuff = thisBuff.slice(Number(len), thisBuff.length)
                            }
                        }
                    }
                    else {
    ...
    getIsoJSON(buffer){
        let incomingLen1 = buffer.slice(0,1)
        let incomingLen2 = buffer.slice(1,2)
        let mti = buffer.slice(2, 6).toString()
        buffer = buffer.slice(6,buffer.byteLength)
        let iso = {
            "0":mti
        }
        let phase_0_127 = this.unpack_127(buffer,iso)
        if(!phase_0_127.error){
            let phase_valid = validateFields(phase_0_127.json)
            if(phase_valid){
                let phase_0_127_1_63 = this.unpack_127_1_63(phase_0_127.remSlice,phase_0_127.json)
                if(!phase_0_127_1_63.error){
                    let phase_127_valid = validateFields(phase_0_127.json)
                    if(phase_127_valid){
                        //let phase_127_25_final = this.unpack_127_25_1_63(phase_0_127_1_63.remSlice,phase_0_127_1_63.json)
                        return phase_0_127_1_63.json
                    }
                    else {
                        return phase_127_valid
                    }
                }
                else {
                    return phase_0_127_1_63
                }
    ...
    assemble127_25_extensions(){
        let buff = Buffer.alloc(8,this.Msg['127.25.1'],'hex');
        console.log(Buffer.alloc(8,this.Msg['127.25.1'],'hex').toString('hex'))
        let bitmaps_127 = this.assembleBitMap_127_125()
        for (let i = 1; i < bitmaps_127.length; i++) {
            let field = '127.25.'+ (Number(i) + 1);
            if (bitmaps_127[i] === 1) {
                if(field ==='127.25.1'){
                    continue
        ...
        let len_0_127_1 = this.getTCPHeaderBuffer(parseInt((Number(buff.byteLength) /256)))
        let len_0_127_2 = this.getTCPHeaderBuffer((parseInt((Number(buff.byteLength)  % 256))))
        return Buffer.concat([len_0_127_1,len_0_127_2,buff])
    }
    ...
    // ***tested***
    getBufferMessage () {
       let state = this.validateMessage()
        if(!state.error){
            let fields_0_127_Buffer = this.assemble0_127_Fields();
            let fields_0_127_0_63_Buffer = this.assemble127_extensions();
            let fields_0_127_25_0_63_Buffer = this.assemble127_25_extensions();

            if(!fields_0_127_Buffer.error && !fields_0_127_0_63_Buffer.error && !fields_0_127_25_0_63_Buffer.error ){
                let len = 4+fields_0_127_Buffer.byteLength+fields_0_127_0_63_Buffer.byteLength+fields_0_127_25_0_63_Buffer.byteLength

                let len_0_127_1 = this.getTCPHeaderBuffer(parseInt((Number(len) /256)))
                let len_0_127_2 = this.getTCPHeaderBuffer((parseInt((Number(len)  % 256))))
                return Buffer.concat([len_0_127_1, len_0_127_2,mtiBuffer,fields_0_127_Buffer,fields_0_127_0_63_Buffer,fields_0_127_25_0_63_Buffer])
            }
       }
       else{
            return state
