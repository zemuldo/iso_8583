...
class Main {
    ...
    getIsoJSON(incoming) {
        if(Buffer.isBuffer(incoming)){
            ...
            let dataString = incoming.slice(22, incoming.length)
            let isoJSON = {
                '0': mti
            }
                    ...
                    if (formats[field]) {
                        if (formats[field].LenType=="fixed") {
                            if(formats[field].ContentType==="b"){
                                isoJSON[field] = dataString.slice(0, formats[field].MaxLen/2).toString('hex')
                                dataString = dataString.slice(formats[field].MaxLen/2, dataString.length)
                            }
                            else {
                                isoJSON[field] = dataString.slice(0, formats[field].MaxLen).toString()
                            ...
                            //check length of iso field
                                ...
                                isoJSON[field] = dataString.slice(0, Number(len)).toString()
    ...
    assembleDataBuffer() {
        ...
        let validDate = validateFields(this.Msg)
        //expects array of 0s & 1s and data-json object
        if (!state.error && !validDate.error && !bitMapCheck.error){
            ...
            return { buff: buff, len: this.dataString.length};
        }
        else {
            return state
        }
    ...
    getBmpPack() {
        ...
        if (mti.error){
            ...
            if(bitMapsHex.error){
                ...
                let data = this.assembleDataBuffer(Buffer.alloc(16,bitMapsHex));
                if(data.error){
                    return data
                }
                else {
    ...
    getBufferMessage () {
        ...
        if (obj.error) {
            ...
            let dataBufferPlusBmpBuffer = this.assembleDataBuffer()
            return Buffer.concat([tcpH1,tcpH2,mtiBuffer,dataBufferPlusBmpBuffer.buff])
        }
    }
