...
let { validateFields,getLenType,getHex } = require('./tools');
...
class Main {
    ...
    //BITMAP FORMAT
    validateMessage(){
        if(this.Msg['0'] && msgTypes[this.Msg['0']]){
            let state = validateFields(this.Msg)
            if(state.error){
                return state
            }
            else{
            ...
            return {error:'mesage undefined or invalid mti'}
        }
    }
    // ***tested***
    checkMTI(){
