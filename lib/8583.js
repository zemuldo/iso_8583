...
class Main {
    ...
    assembleDataElements() {
        ...
        if (!state.error && !validDate.error){
                ...
                if (this.bitmaps[i] === 1) {
                    ...
                    if (formats[field]) {
                        if (formats[field].LenType==="fixed") {
                            if(field === 52){
                                if (formats[field].MaxLen===this.Msg[field].length){
                                    this.dataString += getHex(this.Msg[field]);
                                }
                                else{
                                    return {error:"invalid length of data on field "+field}
                                }
            ...
            return { dataString: this.dataString, len: this.dataString.length, f_52Present:true };
        }
        else {
            return state
        }
    ...
    assembleDataBuffer(bitmapBuffer) {
        let buff = bitmapBuffer
        this.dataString = ''
        let state = this.assembleBitMap()
        let validDate = validateFields(this.Msg)
        //expects array of 0s & 1s and data-json object
        if (!state.error && !validDate.error){
            ...
            for (let i = 1; i < this.bitmaps.length; i++) {
                ...
                if (this.bitmaps[i] === 1) {
                    ...
                    if (formats[field]) {
                        if (formats[field].LenType==="fixed") {
                            if(field[field.ContentType==="b"]){
                                if (formats[field].MaxLen===this.Msg[field].length){
                                   let thisBuff = Buffer.alloc(formats[field].MaxLen,this.Msg[field],'hex')
    ...
    getBufferMessage () {
        ...
        if (obj.error) {
            ...
            let dataBufferPlusBmpBuffer = this.assembleDataBuffer(bitmapBuffer)
