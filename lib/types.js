...
types['b'] = function(options, data) {
  let state = false;
  for (let i = 0; i < data.length; i++)
    if (data[i].length === 1 && data[i].match(regex['b']))
      state = true;
    else
      return false;
...
module.exports = function(options, data, field) {
  if (!types[options.ContentType]) {
    ...
    if (types[options.ContentType](options, data))
      return true;
    else
      return { error: 'while processing field ' + field + ': provided data is not of type \'' + options.ContentType + '\''};
  }
};
